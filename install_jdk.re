...
//image[win-jdk8-download1][Java SE Development Kit 8 Downloads][scale=0.25]{
//}

JDKのダウンロードページ（@<img>{win-jdk8-download1}）が表示されます。
"Java SE Downloads"の左側にある"Java"から「Java Platform (JDK)」をクリックすると@<img>{win-jdk8-download2}が表示されます。

//image[win-jdk8-download2][Java SE Development Kit 8 Downloads][scale=0.25]{
//}
...
ダウンロード後の手順は、使用しているブラウザによって異なります。
例えばMicrosoft Edgeの場合、「実行」するか「ダウンロードしたファイルのあるフォルダをエクスプローラーで開く（ダウンロードの表示）」か、尋ねる画面が表示されます。

お使いのPCにウイルス対策ソフトやセキュリティソフトをインストールしている場合は、それらが警告を表示したり、JDKのダウンロードや、ダウンロードしたファイルが実行できない場合があります。
...
JDKをインストールします。
ダウンロードしたインストールパッケージをダブルクリックで実行します。
...
//footnote[jdk_path][JDKは、標準でC:\Program Files\Java\jdk1.8.0_77\にインストールされます]

入力を終えたら「OK」をクリックし、これまで開いたウィンドウを全て閉じます。
...
//image[mac-jdk8-install5][JDKセットアップ][scale=0.45]{
//}
