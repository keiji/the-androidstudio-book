...
Android Studioは、米Google社が開発しているスマートフォン向けのプラットフォーム（OS）「Android」で動作するソフトウェアを開発するためのIDE（統合開発環境）です。
...
ADTはEclipseベースの統合開発環境で、2007年11月にAndroidが発表になってから2013年の4月までのおよそ５年半、
...
いくら便利な機能があっても、アプリ開発の現場では安定して使えることが重要となります。
...
//indepimage[fb20140908][][scale=0.3]

アップデートの度に画面の構成が変わる。ファイルを開いたときの見た目が変わる。
便利になるのは良いのですが、バージョンが安定しない。
まるで成長期の子供のように落ち着きがありません。

//indepimage[fb20140925][][scale=0.3]

結局、Android Studioのバージョンアップのスピードについて行けるのは、一握りの開発者に限定されてしまいました。


=== 書籍執筆者の苦悩

一般の開発者以上に苦悩していたのが、Android開発を題材にした書籍の執筆者でしょう（ここでは私のことです）。
...
既にADTが古い開発環境となりつつあるのに、Android Studioという良い選択肢があるにも関わらず、
Android Studioを使って学べる本がありません。
...
そのニュースが飛び込んできたのは、脱稿し、急ピッチで編集を進めている中での出来事です。
...
//indepimage[fb20141102][][scale=0.3]


=== 補足資料の公開

書籍側を変更するのが無理なら、補足として正しい情報を広めるしかありません。それも出来るだけ早くにです。
そこで、新しいセットアップ方法について書き下ろし、クリエイティブコモンズ2.1の表示—非営利—改変禁止@<fn>{license}ライセンスの元で配布することにしました。

//indepimage[fb20141111][][scale=0.3]

12月10日にはAndroid Studio 1.0がリリースされ、ADTが表舞台から去りました。
Android公式の開発環境として、Android Studioは、今後ますます重要に重要になってくるでしょう。
